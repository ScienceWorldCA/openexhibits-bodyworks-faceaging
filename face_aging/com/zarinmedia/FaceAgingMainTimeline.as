	...
	public class FaceAgingMainTimeline extends MovieClip {
		...
		internal function startImageSequence():void {
			...
			if (imageSequenceData.numberOfImageSequences() > 5 ) {// leave things blank if we dont have at least 6 image sequences to show
				...
				for (var i:int = 0; i < 6; i++) {
					...
					imageSequenceHolder.addChild(tempISObj)
					imageSequenceClipArray.push(tempISObj );
					tempISObj.x = Point(introImageSequenceLocation[i]).x ;
					tempISObj.y = Point(introImageSequenceLocation[i]).y ;
					tempISObj.name = "imageSequenceMovieclip" + i; // like to name the clips..	
					
					
					if (i == 0) {
							tempISObj.loadSequence(i); // play the first one
						}else {
							tempISObj.loadSequence(i, false); // pause on first frame for all the others
							
						}
					
					tempISObj.addEventListener("COMPLETE", onImageSquenceComplete);
		...
		private function killStep(i:int):void {
			...
			if (i == 3) {
				...
				aDisp.kill();
				removeChild(aDisp); // remove the slider and splitscreen element
				try{
		...
		private function checkStep1ButtonEnable():void {
			if (aprilParameters.ethnicity == undefined || aprilParameters.gender == undefined || aprilParameters.age < 6 || aprilParameters.age > 70) {
	...
	if (buttonPressed == "emailwindowclose" || buttonPressed == "clickoutsideemailmodal" ) {
		...
		private function nextLoaderFactImage():void {
			...
			if (lastimagenumber < 0 ) { // if its the first time.. just play it..
				TweenLite.to(loaderFactJPGArray[loaderFactShowingNow], FaceAgingMainTimeline.configData.getLoaderImageSliderFadeTiming(), { alpha:1 , ease:Quart.easeOut } );

			}else {
				// if its cycling.. remove the last slide before bringing in the next one..
				TweenLite.to(loaderFactJPGArray[loaderFactShowingNow], FaceAgingMainTimeline.configData.getLoaderImageSliderFadeTiming(), { alpha:1 , ease:Quart.easeOut, delay:FaceAgingMainTimeline.configData.getLoaderImageSliderFadeTiming()  } );
				TweenLite.to(loaderFactJPGArray[lastimagenumber], FaceAgingMainTimeline.configData.getLoaderImageSliderFadeTiming(), { alpha:0 , ease:Quart.easeOut } );
			}
		...
		private function savePhoto(bmpData:BitmapData):void {  /// NOTE: this gets called from the Flash timeline.. (dont get confused) had to do it that way as it freezes the system.. so I wanted to give time to the loader elements
			...
			imgBytes = jEncoder.encode(bmpData)
			//var filePath:File = File.documentsDirectory.resolvePath("april_capture.jpg");
			photoCaptureFilePath = File.documentsDirectory.resolvePath("April Age\\april_capture.jpg");
			var stream:FileStream = new FileStream();
			stream.addEventListener(Event.CLOSE , onFinishedSavingPhoto); // this doesnt seem to work , but after the close we seem to have written the file..
			stream.openAsync(photoCaptureFilePath, FileMode.WRITE);
		...
		private function onFinishedSavingPhoto(e:Event):void { // not working see whats up with this.. it should be firing when the photo is saved..
